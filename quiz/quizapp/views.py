...
from questions_app.models import questions
from answers_app.models import Answers
...
def quizform_views(request):
    if request.method == "POST":
        quiztitle = request.POST['quiztitle']
...
def myquizzes_views(request):
    quizzes = Quiz.objects.filter(created_by=request.user)
    return render(request,'myquiz.html',{'quizzes':quizzes})
...
def takequiz_views(request, quiz_id):
...
def result_views(request, quiz_id):
    quiz_object = get_object_or_404(Quiz, id=quiz_id)
...
def analytics_views(request):
        ...
        for quiz in quizzes:
            ...
            total_correct += score
            total_answers += answers
        average = (total_correct / total_answers) * 100
        total_attempts = 0
        if TotalParticipants.objects.filter(user=request.user).exists():
            total_attempts = TotalParticipants.objects.filter(user=request.user).first().total_participants
        chart_labels = []
        ...
        chart_scores = []
        for quiz in quizzes:
            total_correct_for_this_quiz = len(Answers.objects.filter(quiz=quiz, score=1))
            total_answers_for_this_quiz = len(Answers.objects.filter(quiz=quiz))
            average_for_this_quiz = (total_correct_for_this_quiz/total_answers_for_this_quiz) * 100
            chart_scores.append(average_for_this_quiz)
        return render(request, "analytics.html", {
            "quiz_count": quiz_count,
            "total_attempts": total_attempts,
            "overall_avg_score": average,
